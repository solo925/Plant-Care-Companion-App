...
PlantController.get('/all', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const plantRepository = AppDataSource.getRepository(Plant);
        const [plants, total] = await plantRepository
...
PlantController.get('/', verifyToken, async (req: R, res: Response): Promise<void> => {
    ...
    try {
        ...
        const [plants, total] = await plantRepository
            .createQueryBuilder('plant')
            .where('plant.userId = :userId', { userId: req.user?.id })
            .take(limit)
            .skip((page - 1) * limit)
            .getManyAndCount();
    ...
    async (req: R, res: Response): Promise<void> => {
        ...
        try {
            ...
            const user = await userRepository
                ...
                .leftJoinAndSelect('user.ownedPlants', 'ownedPlant')
                .getOne();
    ...
    async (req: R, res: Response): Promise<void> => {
        ...
        try {
            ...
            const user = await userRepository
                .createQueryBuilder('user')
                .where('user.id = :id', { id: userId })
                .leftJoinAndSelect('user.ownedPlants', 'ownedPlants')
                .getOne();
