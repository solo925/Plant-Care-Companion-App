...
PlantHealthLogController.post('/', verifyToken, async (req: Request, res: Response) => {
    ...
    try {
        ...
        const plant = await AppDataSource.getRepository(Plant)
        ...
        const healthLog = healthLogRepository.create({
            plant,
            user,
            healthStatus,
            percentage,
...
PlantHealthLogController.get('/', verifyToken, async (req: Request, res: Response) => {
    try {
        const healthLogs = await AppDataSource.getRepository(PlantHealthLog)
            .createQueryBuilder('healthLog')
            .leftJoinAndSelect('healthLog.plant', 'plant')
            .leftJoinAndSelect('healthLog.user', 'user')
            .orderBy('healthLog.observationDate', 'DESC')
            .getMany();
...
PlantHealthLogController.get('/:plantId', verifyToken, async (req: Request, res: Response) => {
    ...
    try {
        const healthLogs = await AppDataSource.getRepository(PlantHealthLog)
            .createQueryBuilder('healthLog')
            .leftJoinAndSelect('healthLog.plant', 'plant')
            .where('healthLog.plantId = :plantId', { plantId })
            .orderBy('healthLog.observationDate', 'DESC')
            .getMany();
...
PlantHealthLogController.delete('/:logId', verifyToken, async (req: Request, res: Response) => {
    ...
    try {
        const healthLog = await AppDataSource.getRepository(PlantHealthLog)
            .createQueryBuilder('healthLog')
            .where('healthLog.id = :logId', { logId })
            .getOne();
        ...
        await AppDataSource.getRepository(PlantHealthLog)
            .createQueryBuilder()
            .delete()
            .from(PlantHealthLog)
